...
def load_trial_data(trial_directory):
    ...
    csv_files = {
        ...
        'control_loop_timing': 'control_loop_timing.csv'
    }
...
def plot_system_response(data, trial_name):
    ...
    figure, (ax1, ax2, ax3, ax4, ax5, ax6, ax7) = plt.subplots(7, 1, figsize=(12, 28))
    ...
    if data['observed_state_history'] is not None:
        ...
        # Plot control loop timing on the seventh subplot
        if data['control_loop_timing'] is not None:
            timing_data = data['control_loop_timing'].flatten()
            ax7.plot(timing_data, label='Control Loop Execution Time', color='red')
            ...
            ax7.axhline(y=mean_time, color='blue', linestyle='--', alpha=0.7, label=f'Mean: {mean_time:.2f} ms')
            ax7.axhline(y=max_time, color='orange', linestyle=':', alpha=0.7, label=f'Max: {max_time:.2f} ms')
            ax7.axhline(y=min_time, color='green', linestyle=':', alpha=0.7, label=f'Min: {min_time:.2f} ms')
            
            ax7.set_title(f'Control Loop Execution Time (Mean: {mean_time:.2f}±{std_time:.2f} ms)')
            ax7.set_ylabel('Execution Time (ms)')
            ax7.set_xlabel('Time Steps')
            ax7.legend()
            ax7.grid(True, alpha=0.3)
        else:
            ax7.text(0.5, 0.5, 'Control Loop Timing Data Not Available', 
                    horizontalalignment='center', verticalalignment='center',
                    transform=ax7.transAxes, fontsize=12)
            ax7.set_title('Control Loop Execution Time')
            ax7.set_ylabel('Execution Time (ms)')
            ax7.set_xlabel('Time Steps')
