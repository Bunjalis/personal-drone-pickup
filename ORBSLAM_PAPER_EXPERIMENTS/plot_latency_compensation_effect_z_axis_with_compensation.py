...
OBSERVED_STATE_COLOR = '#415395'        # Medium blue - observed state
ESTIMATED_STATE_COLOR = '#826af2'       # Blue-gray - estimated state  
MOTION_CAPTURE_COLOR = '#d78700'        # Green - motion capture data
...
def main():
    ...
    # Create a figure with 4x1 grid of subplots (4 rows, 1 column) with custom height ratios
    # Height ratios: [1, 1, 0.75, 0.75] means first two plots are full height, last two are 3/4 height
    figure, (ax1, ax2, ax3, ax4) = plt.subplots(4, 1, figsize=(12, 24), gridspec_kw={'height_ratios': [1, 1, 0.75, 0.75]})
    ...
    def plot_height_comparison(ax, data_enabled):
        ...
        if data_enabled['estimated_state_history'] is not None and data_enabled['estimated_state_history'].shape[1] >= 3:
            ...
            ax.plot(time_estimated, estimated_height, label=ESTIMATED_HEIGHT_LABEL, color=ESTIMATED_STATE_COLOR, 
                   linestyle=ESTIMATED_STATE_STYLE, linewidth=LINE_WIDTH)
    ...
    def plot_vertical_velocity(ax, data_enabled):
        ...
        if data_enabled['estimated_state_history'] is not None and data_enabled['estimated_state_history'].shape[1] >= 10:
            ...
            ax.plot(time_estimated, estimated_velocity_z, label=ESTIMATED_VELOCITY_LABEL, color=ESTIMATED_STATE_COLOR, 
                   linestyle=ESTIMATED_STATE_STYLE, linewidth=LINE_WIDTH)
    ...
    plt.savefig('latency_comparison_with_compensation.pdf', format='pdf', dpi=400, bbox_inches='tight')
    print("Plot saved as 'latency_comparison_with_compensation.pdf'")
