...
class ELRSInterface(Node):
    ...
    def controller_commands_callback(self, msg):
        ...
        if self.armed == True:  
            ...
            #self.roll = self.middle_value + int(roll_input * 1000)
            #self.pitch = self.middle_value + int(pitch_input * 1000)
            #self.yaw = self.middle_value + int(yaw_input * 1000)
            #self.throttle = int(throttle_input * 2000)
