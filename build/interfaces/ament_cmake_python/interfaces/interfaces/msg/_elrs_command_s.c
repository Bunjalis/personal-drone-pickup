...
bool interfaces__msg__elrs_command__convert_from_py(PyObject * _pymsg, void * _ros_message)
{
  ...
  {  // channel_11
    PyObject * field = PyObject_GetAttrString(_pymsg, "channel_11");
    if (!field) {
      return false;
    }
    assert(PyFloat_Check(field));
    ros_message->channel_11 = (float)PyFloat_AS_DOUBLE(field);
    Py_DECREF(field);
  }
...
PyObject * interfaces__msg__elrs_command__convert_to_py(void * raw_ros_message)
{
  ...
  {  // channel_11
    PyObject * field = NULL;
    field = PyFloat_FromDouble(ros_message->channel_11);
    {
      int rc = PyObject_SetAttrString(_pymessage, "channel_11", field);
      Py_DECREF(field);
      if (rc) {
        return NULL;
      }
