...
class QuadDynamics:
    ...
    def v_dynamics(self):
        ...
        motor_thrusts = cs.sign(self.actuators) * self.thrust_constant * cs.power(max_rpm * cs.fabs(self.actuators), 2)
    ...
    def w_dynamics(self):
        # Correct quadratic thrust model for angular dynamics with bidirectional control
        max_rpm = 4631.0
        
        # Calculate thrust forces for each motor using actuator states
        # T = sign(actuator) * k * (max_rpm * |actuator|)^2
        thrusts = cs.sign(self.actuators) * self.thrust_constant * cs.power(max_rpm * cs.fabs(self.actuators), 2)
        ...
        for i in range(8):
            ...
            thrust_i = cs.sign(self.actuators[i]) * self.thrust_constant * cs.power(max_rpm * cs.fabs(self.actuators[i]), 2)
