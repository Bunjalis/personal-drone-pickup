...
class QuadDynamics:
    ...
    def v_dynamics(self):
        ...
        # Construct thrust constant from adaptable base and fixed scale
        # thrust_constant = thrust_base * 1e-7
        effective_thrust_constant = self.thrust_base * self.thrust_scale
        motor_thrusts = cs.sign(self.actuators) * effective_thrust_constant * cs.power(self.max_rpm * cs.fabs(self.actuators), 2)
        f_thrust = cs.mtimes(self.mot_rot_vec.T, motor_thrusts)
        
        g = cs.vertcat(0.0, 0.0, 9.81)
        v_dynamics = self.v_dot_q(f_thrust, self.q) / self.mass_constant - g
    ...
    def w_dynamics(self):
        ...
        # T = sign(actuator) * (thrust_base * 1e-7) * (max_rpm * |actuator|)^2
        effective_thrust_constant = self.thrust_base * self.thrust_scale
        thrusts = cs.sign(self.actuators) * effective_thrust_constant * cs.power(self.max_rpm * cs.fabs(self.actuators), 2)

        # Compute torques generated by each motor (thrust-induced torques)
        thrust_torque = cs.MX.zeros(3)
        for i in range(8):
            # Scale motor position by the fixed motor_distance
            motor_pos = self.motor_distance_fixed * self.mot_pos_vec_base[i, :]
            thrust_torque += thrusts[i] * cs.cross(motor_pos, self.mot_rot_vec[i, :])


        # Total torque = thrust-induced torques + adaptive offset moments
        adaptive_moments = cs.vertcat(self.theta_roll, self.theta_pitch, self.theta_yaw)
        total_torque = thrust_torque + adaptive_moments

        # Compute angular acceleration using fixed inertia values
        J_inv = cs.diag(1 / self.J)  # Inverse of fixed inertia matrix
