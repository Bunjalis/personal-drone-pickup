...
class Controller(Node):
    def __init__(self):
        ...
        self.est_params = np.array([38.0, 0.1, 0.0, 0.0], dtype=float)
        ...
        self.P = np.diag([
            ...
            0.1, 0.001,                 # kT, dragZ
            0.1, 0.1                    # fc_roll_offset_deg, fc_pitch_offset_deg
        ]).astype(float)
        ...
        self.Q = np.diag([
            ...
            # params (slower drift)
            1e-3, 1e-5,                 # kT, dragZ
            1e-5, 1e-5                  # fc_roll_offset_deg, fc_pitch_offset_deg
        ]).astype(float)

        # Measurement: 10 (p(3), yaw(1), v(3), w(3))
        self.R = np.diag([0.05] * 10).astype(float)

        # Delay estimation (number of control steps of SLAM delay)
        self.delay_states = 3
        ...
        log_headers = [
            ...
            'orb_pose_x', 'orb_pose_y', 'orb_pose_z',
            'orb_pose_qw', 'orb_pose_qx', 'orb_pose_qy', 'orb_pose_qz',
            'orb_pose_vx', 'orb_pose_vy', 'orb_pose_vz',
            'orb_pose_avx', 'orb_pose_avy', 'orb_pose_avz',

            'mot_pose_x', 'mot_pose_y', 'mot_pose_z',
            'mot_pose_qw', 'mot_pose_qx', 'mot_pose_qy', 'mot_pose_qz',
            'mot_pose_vx', 'mot_pose_vy', 'mot_pose_vz',
            'mot_pose_avx', 'mot_pose_avy', 'mot_pose_avz',

            'est_pose_x', 'est_pose_y', 'est_pose_z',
            'est_pose_qw', 'est_pose_qx', 'est_pose_qy', 'est_pose_qz',
            'est_pose_vx', 'est_pose_vy', 'est_pose_vz',
            'est_pose_avx', 'est_pose_avy', 'est_pose_avz',

            'ukf_pose_x', 'ukf_pose_y', 'ukf_pose_z',
            'ukf_pose_qw', 'ukf_pose_qx', 'ukf_pose_qy', 'ukf_pose_qz',
            'ukf_pose_vx', 'ukf_pose_vy', 'ukf_pose_vz',
            'ukf_pose_avx', 'ukf_pose_avy', 'ukf_pose_avz',
    ...
    def control_loop(self):
        ...
        if self.armed and self.current_pose is not None:
            ...
            # ---- Delay-compensated measurement state for UKF (using sim_integrator) ----
            # Start from latest ORB-SLAM pose, but initialise orientation with UKF quaternion
            meas_state = copy.deepcopy(self.current_pose[:13])
            meas_state[3:7] = self.x_est[3:7]
            ...
            for val in delayed_control_history:
                # integrator state = [p(3), q(4), v(3), w(3), u(4)]
                self.sim_integrator.set(
                    "x",
                    np.concatenate((meas_state, np.array(val[0:4]).flatten()))
                )
                ...
                meas_state = x_next[:13]

            # Build hybrid quaternion for measurement: UKF roll/pitch + SLAM yaw (after delay-comp)
            qw_meas, qx_meas, qy_meas, qz_meas = meas_state[3:7]
            ...
            meas_state[3:7] = np.array([hybrid_qw, hybrid_qx, hybrid_qy, hybrid_qz]) / quat_norm

            # ---- MPC initial state: use UKF filtered state directly ----
            mpc_state = self.x_est[:13].copy()

            if len(self.control_history) == 0:
                self.get_logger().warn("Control history is empty - cannot set state bounds accurately")
                mpc_state_with_control = np.concatenate((mpc_state, np.array([0.0, 0.0, 0.0, 0.0])))
            else:
                mpc_state_with_control = np.concatenate(
                    (mpc_state, np.array(self.control_history[-1][0:4]))
                )

            init_mpc_state = mpc_state_with_control.copy()
            ...
            if self.takeoff_requested:
                # 1) Prediction: use the *latest* applied control as the input
                if len(self.control_history) == 0:
                    u_pred = np.zeros(4)
                    u_rate_pred = np.zeros(4)
                else:
                    last_ctrl = self.control_history[-1]
                    u_pred = np.array(last_ctrl[0:4])
                    u_rate_pred = np.array(last_ctrl[4:8])

                sigma_pts, wm, wc = self.generate_sigma_points(
                    self.x_est, self.P, self.alpha, self.beta, self.kappa
                )
                sigma_pts_pred = np.array([self.fx(pt, u_pred, u_rate_pred) for pt in sigma_pts])
                x_pred, P_pred = self.unscented_transform(sigma_pts_pred, wm, wc, self.Q)

                # 2) Measurement transform
                ...
                # 3) Measurement: use the delay-compensated hybrid state `meas_state`
                qw_m, qx_m, qy_m, qz_m = meas_state[3:7]
                yaw_measured = np.arctan2(
                    2.0 * (qw_m * qz_m + qx_m * qy_m),
                    1.0 - 2.0 * (qy_m ** 2 + qz_m ** 2)
                )

                z_measured = np.concatenate([
                    meas_state[0:3],      # position (delay-compensated)
                    [yaw_measured],       # yaw from hybrid quaternion
                    meas_state[7:10],     # velocity
                    meas_state[10:13],    # angular velocity
                ])
                ...
                quat_norm_state = np.linalg.norm(self.x_est[3:7])
                if quat_norm_state > 0:
                    self.x_est[3:7] = self.x_est[3:7] / quat_norm_state
            ...
            log_row = [
                ...
                # est_pose_* → use delay-compensated measurement state (meas_state)
                float(meas_state[0]), float(meas_state[1]), float(meas_state[2]),
                float(meas_state[3]), float(meas_state[4]), float(meas_state[5]), float(meas_state[6]),
                float(meas_state[7]), float(meas_state[8]), float(meas_state[9]),
                float(meas_state[10]), float(meas_state[11]), float(meas_state[12]),

                # ukf_pose_* → filter state
            ...
            self.estimated_state_history.append(meas_state[:13].tolist())
