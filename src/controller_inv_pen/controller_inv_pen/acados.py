...
def generate_ocp_controller(dynamics=None):
    ...
    model.x = quad_dynamics.x  # Include omega in the state vector # Defines state vector
    model.u = quad_dynamics.u # Defines control action
    model.f_expl_expr = dynamics_expr(quad_dynamics.x, quad_dynamics.u)  # Use full state including omega internally # Defines state space model
    ...
    ocp.solver_options.N_horizon = 20 # Set time/control horizon 
    ocp.solver_options.tf = 2.0 # prediction horizon for optimization
    ...
    ocp.cost.Vx = np.zeros((ny, nx)) # v matrix coefficient at intermediate shooting nodes 
    ocp.cost.Vx[:nx, :nx] = 1 * np.eye(nx) 
    ocp.cost.Vu = np.zeros((ny, nu)) # u matrix coefficient at intermediate shooting nodes 
    ocp.cost.Vu[-4:, -4:] = 1 * np.eye(nu)
    ocp.cost.Vx_e = np.eye(nx) # Terminal coefficient matrix

    ocp.cost.yref = np.zeros((nx + nu, )) # reference intermediate shooting node
